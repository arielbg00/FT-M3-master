...
function problemA () {
  ...
  // AsyncAwait version
  let readAsync = async (file) => {
    blue(await promisifiedReadFile(file));
  };

  async function withoutOrder() {
...
function problemB () {
  ...
  let arrayPromises = filenames.map(f => readAsync(f));
  ...
  withoutOrder();
  // const res = await Promise.all(filenames.map(fn => promisifiedReadFile(fn)));
  // for (const r of res) {
  //   blue(r);
  // }
  // console.log("done");
